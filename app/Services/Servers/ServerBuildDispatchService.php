...
class ServerBuildDispatchService
{
    public function build(ServerDeploymentData $deployment): void
    {
        ...
        Bus::chain($jobs)
            ->catch(function (\Throwable $e) use ($deployment) {
                \Illuminate\Support\Facades\Log::error("BuildServerJob chain failed for Server ID {$deployment->server->id} (VMID: {$deployment->server->vmid}): {$e->getMessage()}", [
                    'exception' => get_class($e),
                    'file' => $e->getFile(),
                    'line' => $e->getLine(),
                    'trace' => $e->getTraceAsString(),
                ]);
                $deployment->server->update(['status' => Status::INSTALL_FAILED->value]);
            })
            ->dispatch();
