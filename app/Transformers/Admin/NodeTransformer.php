...
class NodeTransformer extends TransformerAbstract
{
    public function transform(Node $node): array
    {
        ...
        } elseif (Str::contains(strtolower($locCode), 'sg')) {
            $flag = 'https://flagcdn.com/sg.svg';
        } elseif (Str::contains(strtolower($locCode), 'au')) {
        ...
        return [
            ...
            // ssh_private_key is write-only — omitted from GET responses for security.
            'ssh_host'            => $node->ssh_host,
            'ssh_port'            => $node->ssh_port ?? 22,
            'ssh_username'        => $node->ssh_username ?? 'root',
            'backup_path'         => $node->backup_path,
            'ssh_key_configured'  => !empty($node->getRawOriginal('ssh_private_key')),
        ];
    }
}
