...
perform_update() {
    ...
    COMPOSER_LOG="/tmp/vertex_composer.log"
    ...
    if composer install --no-dev --optimize-autoloader --no-interaction \
        ...
        success "PHP dependencies & autoloader updated"

    else

        # Fallback in case composer.lock was somehow out of sync
