...
import { MegaphoneIcon, CheckCircleIcon, XCircleIcon } from '@heroicons/react/24/outline'
import http from '@/api/http'

export const AdminAnnouncementToggle: React.FC = () => {
    ...
    const fetchSetting = async () => {
        setLoading(true)
        try {
            const res = await http.get('/api/admin/settings/announcement')
    ...
    const handleToggle = async () => {
        ...
        setUpdating(true)
        try {
            const res = await http.post('/api/admin/settings/announcement', {
                enabled: nextState,
            })
            ...
            alert('Failed to update announcement row setting.')
        } finally {
            setUpdating(false)
        }
    }
                ...
                <div className='flex items-start gap-4'>
                    ...
                    <div>
                            ...
                            {!loading && (
                                enabled ? (
                                    <span className='px-3 py-0.5 rounded-full text-xs font-bold bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 flex items-center gap-1.5 backdrop-blur-md'>
                                        <CheckCircleIcon className='w-4 h-4' /> Enabled & Visible
                                    </span>
                                ) : (
                        ...
                        <p className='text-xs text-gray-400 mt-1 max-w-xl leading-relaxed'>
                            Control visibility of the top promo sale and referral banner row displayed on client user dashboards.
                        </p>
                    </div>
                </div>
                ...
                <div className='flex items-center gap-3 shrink-0'>
                    ...
                    >
                        {updating
                            ? 'Updating...'
                            : enabled
                            ? 'Disable Announcement Row'
                            : 'Enable Announcement Row'}
                    </button>
                </div>
            </div>
        </div>
...
export default AdminAnnouncementToggle
